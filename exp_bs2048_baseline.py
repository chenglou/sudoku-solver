# Baseline with BS=2048 — same as exp_faster_2drope but batch_size=2048.
# Purpose: isolate the effect of halving batch size from other changes.
# Everything else identical: 16 iters, reverse curriculum, cosine LR.
...
CHECKPOINT_PREFIX = "bs2048_baseline_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_baseline',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_baseline.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 baseline (same as exp_faster_2drope but BS=2048)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_baseline")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"2D RoPE baseline (BS=4096): 82.5%")

    final_path = os.path.join(output_dir, "model_bs2048_baseline.pt")
