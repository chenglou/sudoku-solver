# RRN Experiment: Reverse curriculum (hard -> easy) on sudoku-extreme
...
CHECKPOINT_PREFIX = "rrn_reverse_curriculum_checkpoint_step"

CONFIG = {
    'experiment': 'rrn_exp_reverse_curriculum',
    'hidden_size': 128,
    'num_steps': 16,
    'batch_size': 1024,
    'lr': 2e-3,
...
# Reverse curriculum phases (hard -> easy) - same as transformer
PHASES = [
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: RRN Reverse Curriculum (hard->easy)")
    print(f"Architecture: hidden_size={hidden_size}, num_steps={num_steps}")
    print(f"Batch size: {batch_size}, lr: {lr}")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "rrn_exp_reverse_curriculum.log")
    log_file = open(log_path, "a")
    writer = SummaryWriter(log_dir=os.path.join(output_dir, "runs", "rrn_reverse_curriculum"))
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - RRN Reverse Curriculum")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_rrn_reverse_curriculum.pt")
