# Recurrence Experiment 2: Concatenate previous hidden state to input
# input_proj: (10 + 9 + d_model) -> d_model
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # NEW: input_proj takes (10 + 9 + d_model) instead of (10 + 9)
        self.input_proj = nn.Linear(10 + 9 + d_model, d_model)
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            # NEW: concatenate h_prev to input
            x_in = torch.cat([x, preds, h_prev], dim=-1)
            h = self.input_proj(x_in)
            h = h + pos_embed
            h = self.transformer(h)
            h_prev = h  # NEW: save for next iteration
...
print(f"\nRecurrence Exp 2: Concat h_prev")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_concat.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence Exp 2: Concat h_prev")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_concat.pt")
log("Model saved to model_recur_concat.pt")
