# Train on sudoku-extreme with reverse curriculum
# Uses no_x_after_init architecture (encodes puzzle once)
...
PHASES = [
    (0, 20000, 21, "Phase 1: Hard only (rating 21+)"),
    (20000, 40000, 6, "Phase 2: Medium+ (rating 6+)"),
    (40000, 60000, 1, "Phase 3: Easy+ (rating 1+)"),
    (60000, 100000, 0, "Phase 4-5: All (rating 0+)"),
]
...
# Split: use first 400K for training, last 22K for test
train_size = 400000
...
for min_r, max_r, name in RATING_BUCKETS:
    ...
    print(f"  Rating {name}: {len(puzzles)} puzzles")
...
test_data = {}
for min_r, max_r, name in RATING_BUCKETS:
    indices = [i for i in range(train_size, len(dataset)) if min_r <= dataset[i]['rating'] <= max_r]
    if len(indices) == 0:
        continue
    puzzles = [dataset[i]['question'] for i in indices]
    solutions = [dataset[i]['answer'] for i in indices]
...
print(f"\nExperiment: sudoku-extreme with reverse curriculum")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_extreme_curriculum.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - sudoku-extreme with reverse curriculum")
log("="*60)
results = evaluate_all()
total_r = results.pop('_total')
...
log(f"TRM comparison: 87.4%")

torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_extreme_curriculum.pt")
log("Model saved to model_extreme_curriculum.pt")
