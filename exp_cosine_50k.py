# Experiment: Shorter training with faster cosine decay
# Based on exp_cosine_no_sam.py (83.6% at 70K steps)
# Changes:
...
# Hypothesis: If reaching low LR is key (not total steps), 50K should match 70K
...
CHECKPOINT_PREFIX = "cosine_50k_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_50k',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
n_iterations = 16
lr = 1.5e-3
warmup_steps = 1400  # Scaled from 2000 * (50/70)
lr_min_ratio = 0.01
total_steps = 50000  # KEY CHANGE: 50K instead of 70K
batch_size = 4096
train_size = 2700000

# Scaled curriculum phases (each ~10K instead of 14K)
PHASES = [
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR with 50K steps (faster decay)")
    print(f"Architecture: d_model={d_model}, n_layers={n_layers}")
    print(f"Batch size: {batch_size}, lr: {lr}, warmup_steps: {warmup_steps}")
    print(f"Total steps: {total_steps} (vs 70K baseline)")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_cosine_50k.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR 50K steps")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"70K baseline: 83.6%")

    final_path = os.path.join(output_dir, "model_cosine_50k.pt")
