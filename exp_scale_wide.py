# Experiment: Scale WIDTH only (keep depth same as baseline)
# Tests hypothesis that doubling n_layers (4->8) hurt convergence
#
# Local:  python exp_scale_wide.py
# Modal:  modal run --detach modal_run.py --exp exp_scale_wide
...
# Hyperparameters - scale WIDTH only (keep depth=4 from baseline)
d_model = 256      # 2x baseline (was 128)
n_heads = 8        # 2x baseline (was 4)
d_ff = 1024        # 2x baseline (was 512)
n_layers = 4       # SAME as baseline (not 8!)
n_iterations = 16
lr = 1e-3
total_steps = 70000
...
def find_latest_checkpoint(output_dir):
    ...
    pattern = os.path.join(output_dir, "scale_wide_checkpoint_step*.pt")
...
def train(output_dir="."):
    """Main training function. Output files go to output_dir."""
    ...
    print(f"\nExperiment: Scale WIDTH only (keep depth=4 from baseline)")
    print(f"Architecture: d_model={d_model}, n_layers={n_layers}, n_heads={n_heads}, d_ff={d_ff}")
    print(f"Iterations: {n_iterations}, batch_size={batch_size}")
    print(f"SAM rho={sam_rho}")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_scale_wide.log")
    ...
    def save_checkpoint(step):
        checkpoint_path = os.path.join(output_dir, f"scale_wide_checkpoint_step{step}.pt")
        torch.save({
            'step': step,
            'config': CONFIG,
    ...
    for step in range(start_step, total_steps):
        ...
        # TRUE batch - no gradient accumulation
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Scale WIDTH only (depth=4)")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Scale-up depth+width (d=256, L=8): 69.7%")
    log(f"TRM (5M params): 87.4%")

    final_model_path = os.path.join(output_dir, "model_scale_wide.pt")
    torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, final_model_path)
    log(f"Final model saved: {final_model_path}")

    log_file.close()

    return {
        'total_solved': total_r['solved'],
        'total_puzzles': total_r['total'],
        'accuracy': total_r['solved'] / total_r['total'],
    }
