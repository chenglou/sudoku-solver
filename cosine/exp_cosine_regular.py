# Experiment: Cosine LR decay + Regular curriculum (easy→hard)
# Based on exp_cosine.py
# Changes:
#   - Regular curriculum instead of reverse: start easy, add harder puzzles
# Hypothesis: Does cosine help regular curriculum avoid forgetting?
...
CHECKPOINT_PREFIX = "cosine_regular_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_regular',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
# max_rating means "include buckets where bucket_max <= max_rating"
PHASES = [
    (0, 14000, 2, "Phase 1: Easy only (rating ≤2)"),
    (14000, 28000, 10, "Phase 2: +Medium (rating ≤10)"),
    (28000, 42000, 50, "Phase 3: +Hard (rating ≤50)"),
    (42000, 70000, 1000, "Phase 4: All (rating ≤1000)"),
]
...
def train(output_dir="."):
    ...
    # Regular curriculum: include buckets where bucket_max <= phase_max_rating
    phase_buckets = {}
    for start, end, max_rating, name in PHASES:
    ...
    log_path = os.path.join(output_dir, "exp_cosine_regular.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR + Regular Curriculum")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_cosine_regular.pt")
