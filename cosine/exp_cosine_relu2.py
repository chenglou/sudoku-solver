...
# Changes:
#   - activation = ReLU^2 (x -> max(0,x)^2)
# Hypothesis: ReLU^2 has shown benefits in some architectures (e.g., Primer)
...
CHECKPOINT_PREFIX = "cosine_relu2_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_relu2',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            activation=relu_squared,  # KEY CHANGE: ReLU^2
        )
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR with ReLU^2 activation")
    print(f"Architecture: d_model={d_model}, n_layers={n_layers}")
    print(f"Batch size: {batch_size}, lr: {lr}")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_cosine_relu2.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR with ReLU^2")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_cosine_relu2.pt")
