# Experiment: High weight decay (matching nano-trm)
# Based on exp_cosine_no_sam.py (83.6% SOTA)
# Changes:
#   - weight_decay=1.0 (vs default 0.01, 100x higher)
# Hypothesis: Higher weight decay provides stronger regularization like nano-trm
...
CHECKPOINT_PREFIX = "cosine_wd_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_wd',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
    'warmup_steps': 2000,
    'lr_min_ratio': 0.01,
    'weight_decay': 1.0,  # nano-trm uses 1.0, we were using 0.01 (default)
}
...
batch_size = 4096
train_size = 2700000
weight_decay = 1.0  # KEY CHANGE: 100x higher than default
...
def train(output_dir="."):
    ...
    # AdamW with HIGH weight decay (nano-trm uses 1.0)
    ...
    print(f"\nExperiment: Cosine LR with HIGH weight decay")
    ...
    log_path = os.path.join(output_dir, "exp_cosine_wd.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR with HIGH weight decay")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Baseline (wd=0.01): 83.6%")

    final_path = os.path.join(output_dir, "model_cosine_wd.pt")
