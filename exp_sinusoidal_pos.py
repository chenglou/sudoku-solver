# Experiment: Hard-coded sinusoidal positional encoding
# Change: Replace learned row/col/box embeddings with sinusoidal encodings
import torch
...
# Precompute sinusoidal encodings for row/col/box (0-8 each)
ROW_PE = sinusoidal_encoding(9, d_model)  # (9, d_model)
COL_PE = sinusoidal_encoding(9, d_model)  # (9, d_model)
BOX_PE = sinusoidal_encoding(9, d_model)  # (9, d_model)
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # Register sinusoidal encodings as buffers (not learned)
        self.register_buffer('row_pe', ROW_PE)
        self.register_buffer('col_pe', COL_PE)
        self.register_buffer('box_pe', BOX_PE)
    ...
    def forward(self, x, return_all=False):
        ...
        # Structured sinusoidal pos encoding: row + col + box
        pos_embed = (self.row_pe[self.row_idx] +
                     self.col_pe[self.col_idx] +
                     self.box_pe[self.box_idx])  # (81, d_model)

        preds = torch.zeros(batch_size, 81, 9, device=x.device)
        all_logits = []
...
print(f"\nEXPERIMENT: Sinusoidal positional encoding (structured: row + col + box)")
print(f"Training on {device}...")

log_file = open("exp_sinusoidal_pos.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
