# Experiment: GELU activation (modern transformer standard)
# Based on exp_cosine_no_sam.py (83.6% SOTA)
# Changes:
#   - activation="gelu" instead of default "relu"
# Hypothesis: GELU may improve performance like in modern transformers
...
CHECKPOINT_PREFIX = "cosine_gelu_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_gelu',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            activation="gelu",  # KEY CHANGE: GELU instead of ReLU
        )
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR with GELU activation")
    print(f"Architecture: d_model={d_model}, n_layers={n_layers}")
    print(f"Batch size: {batch_size}, lr: {lr}")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_cosine_gelu.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR with GELU")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Baseline (ReLU): 83.6%")

    final_path = os.path.join(output_dir, "model_cosine_gelu.pt")
