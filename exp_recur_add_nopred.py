# Recurrence + No Preds: Only use h_prev, remove preds from input
# Since h_prev contains all info (including what led to preds), preds is redundant
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # CHANGED: input_proj only takes puzzle (10 dims), not preds (was 10+9=19)
    ...
    def forward(self, x, return_all=False):
        ...
        h_prev = torch.zeros(batch_size, 81, d_model, device=device)
        all_logits = []
        for _ in range(n_iterations):
            # CHANGED: only use x, not concat(x, preds)
            h = self.input_proj(x)
            h = h + pos_embed + h_prev  # h_prev carries all iteration state
...
print(f"\nRecurrence + No Preds (add variant)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_add_nopred.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence + No Preds (add)")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_add_nopred.pt")
log("Model saved to model_recur_add_nopred.pt")
