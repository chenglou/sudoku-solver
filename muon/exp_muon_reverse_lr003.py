# Experiment: Muon reverse curriculum, lr=3e-3, no weight decay
# Based on exp_muon_reverse_lr002 with slightly higher LR.
...
CHECKPOINT_PREFIX = "muon_reverse_lr003_checkpoint_step"

CONFIG = {
    'experiment': 'exp_muon_reverse_lr003',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_muon_reverse_lr003.log"

# Reverse curriculum (hard -> easy), same as 50K baseline
PHASES = [
...
def train(output_dir="."):
    ...
    print("\nExperiment: Muon reverse curriculum lr=3e-3 (d_model=128, head_dim=32)")
    print(f"Architecture: d_model={d_model}, d_ff={d_ff}, n_layers={n_layers}")
    print(f"Batch size: {batch_size}, lr: {lr}, wd: {weight_decay}, warmup_steps: {warmup_steps}")
    ...
    for step in range(start_step, total_steps):
        ...
        if step % 100 == 0 or step == total_steps - 1:
            ...
            if do_eval:
                ...
                log(
                    f"Step {step:5d} | LR: {current_lr:.2e} | Loss: {loss.item():.4f} "
                    f"Acc: {train_acc:.2%} | " +
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_muon_reverse_lr003")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_muon_reverse_lr003.pt")
