# Experiment: Muon lr=0.02 with grad clipping (phased curriculum)
# Based on exp_muon_lr02 but adds gradient clipping.
...
CHECKPOINT_PREFIX = "muon_lr02_clip_checkpoint_step"

CONFIG = {
    'experiment': 'exp_muon_lr02_clip',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_muon_lr02_clip.log"
grad_clip_norm = 1.0
...
def train(output_dir="."):
    ...
    print("\nExperiment: Muon lr=0.02 + grad clip (d_model=128, head_dim=32)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_muon_lr02_clip")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_muon_lr02_clip.pt")
