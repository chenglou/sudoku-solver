# Experiment: Muon lr=0.02 with mixed sampling
# Based on exp_muon_lr02 but removes curriculum phases.
...
CHECKPOINT_PREFIX = "muon_lr02_mixed_checkpoint_step"

CONFIG = {
    'experiment': 'exp_muon_lr02_mixed',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_muon_lr02_mixed.log"
...
def train(output_dir="."):
    ...
    print("\nExperiment: Muon lr=0.02 mixed (d_model=128, head_dim=32)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_muon_lr02_mixed")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_muon_lr02_mixed.pt")
