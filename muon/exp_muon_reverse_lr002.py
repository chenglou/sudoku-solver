# Experiment: Muon reverse curriculum, lr=2e-3, no weight decay
# Based on exp_muon_again but with slightly higher LR.
...
CHECKPOINT_PREFIX = "muon_reverse_lr002_checkpoint_step"

CONFIG = {
    'experiment': 'exp_muon_reverse_lr002',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_muon_reverse_lr002.log"
...
def train(output_dir="."):
    ...
    print("\nExperiment: Muon reverse curriculum lr=2e-3 (d_model=128, head_dim=32)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_muon_reverse_lr002")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_muon_reverse_lr002.pt")
