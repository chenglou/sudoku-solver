# Experiment: Double batch size, half steps (same data exposure, faster)
# Based on exp_cosine_50k.py (82.8% at 50K steps, BS=4096)
# Changes:
#   - BS=8192 (doubled)
#   - 25K steps (halved) - same total samples seen
#   - Scaled curriculum and warmup
# Hypothesis: Should match 50K results in ~half the time
...
CHECKPOINT_PREFIX = "cosine_25k_bs8k_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_25k_bs8k',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 8192,
    'lr': 1.5e-3,
...
n_iterations = 16
lr = 1.5e-3
warmup_steps = 700  # Scaled from 1400 * (25/50)
lr_min_ratio = 0.01
total_steps = 25000  # Half of 50K
batch_size = 8192  # Double of 4096
train_size = 2700000

# Scaled curriculum phases (each ~5K instead of 10K)
PHASES = [
    (0, 5000, 21, "Phase 1: Hard only (rating 21+)"),
    (5000, 10000, 6, "Phase 2: Medium+ (rating 6+)"),
    (10000, 15000, 1, "Phase 3: Easy+ (rating 1+)"),
    (15000, 25000, 0, "Phase 4: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=8192, 25K steps (2x batch, half steps)")
    ...
    log_path = os.path.join(output_dir, "exp_cosine_25k_bs8k.log")
    ...
    for step in range(start_step, total_steps):
        ...
        if step % 100 == 0 or step == total_steps - 1:
            ...
            if step % 2500 == 0 or step == total_steps - 1:  # Eval every 2.5K (scaled from 5K)
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - BS=8192, 25K steps")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"50K BS=4096 baseline: 82.8%")

    final_path = os.path.join(output_dir, "model_cosine_25k_bs8k.pt")
