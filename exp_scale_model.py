# Scale experiment: Larger model (modest)
# d_model: 128→192, n_layers: 4→6, n_heads: 4→6, d_ff: 512→768
...
# Hyperparameters (modest scale: ~2.4M params vs ~800k baseline)
d_model = 192
n_heads = 6
d_ff = 768
n_layers = 6
...
lr = 1e-3
total_steps = 100000
batch_size = 256  # reduced from 512 to fit in memory
sam_rho = 0.05
...
print(f"\nScale Experiment: Larger Model (d=192, L=6)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_scale_model.txt", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
# Final evaluation
log("\n" + "="*60)
log("FINAL RESULTS - Scale Model (d=192, L=6)")
log("="*60)
results = evaluate_all()
...
# Save model
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_scale_model.pt")
log("Model saved to model_scale_model.pt")
