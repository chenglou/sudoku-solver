# Experiment: Cosine LR decay WITHOUT SAM
# Based on exp_cosine.py (84.0% SOTA)
# Changes:
#   - Remove SAM, use plain AdamW
# Hypothesis: Cosine decay may make SAM redundant for finding flat minima
...
CHECKPOINT_PREFIX = "cosine_no_sam_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_no_sam',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
def train(output_dir="."):
    ...
    log_path = os.path.join(output_dir, "exp_cosine_no_sam.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR WITHOUT SAM")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Cosine WITH SAM: 84.0%")

    final_path = os.path.join(output_dir, "model_cosine_no_sam.pt")
