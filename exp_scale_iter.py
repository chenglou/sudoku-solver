# Scale experiment: More iterations
# n_iterations: 16→32
...
print(f"\nScale Experiment: More Iterations (32)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_scale_iter.txt", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
# Final evaluation
log("\n" + "="*60)
log("FINAL RESULTS - Scale Iterations (32)")
log("="*60)
results = evaluate_all()
...
# Save model
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_scale_iter.pt")
log("Model saved to model_scale_iter.pt")
