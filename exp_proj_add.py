# Experiment: Project-then-add input combination (instead of concat)
# Change: proj_digit(digit) + proj_pred(preds) + empty_embed(is_empty) instead of input_proj(cat([puzzle, preds]))
import torch
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # EXPERIMENT: Project-then-add instead of concat
        self.proj_digit = nn.Linear(9, d_model)      # project digit one-hot
        self.proj_pred = nn.Linear(9, d_model)       # project predictions
        self.empty_embed = nn.Embedding(2, d_model)  # embed is_empty flag
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            # EXPERIMENT: Project-then-add instead of concat
            is_empty = x[:, :, 0].long()       # (batch, 81) - 0 or 1
            digit_onehot = x[:, :, 1:]         # (batch, 81, 9)
            h = self.proj_digit(digit_onehot) + self.proj_pred(preds) + self.empty_embed(is_empty)
            h = h + pos_embed
...
print(f"\nEXPERIMENT: Project-then-add input combination")
print(f"Training on {device}...")

log_file = open("exp_proj_add.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
