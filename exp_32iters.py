# Experiment: 32 iterations training for fixed-point stability
# Hypothesis: training with 16 iters means f never sees correct inputs,
# so the correct solution isn't a stable fixed point. Training with 32 iters
# lets easy puzzles be solved by iter ~10, giving f 20+ iterations of
# "correct input → stay correct" training signal.
#
# Changes from exp_faster_2drope:
# - n_iterations: 16 → 32
# - Mixed sampling (no curriculum) — easy puzzles throughout training
#   provide the stability signal
# - BS=2048 (4096 OOMs on H200 with 32 iters)
...
CHECKPOINT_PREFIX = "32iters_checkpoint_step"

CONFIG = {
    'experiment': 'exp_32iters',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    total_train = sum(train_data[k]['size'] for k in all_buckets)
    print(f"  Mixed sampling: {total_train} puzzles (all difficulties)")
    ...
    print(f"\nExperiment: 32 iterations (stability training)")
    print(f"Architecture: d_model={d_model}, d_ff={d_ff}, n_layers={n_layers}")
    print(f"Iterations: {n_iterations}")
    print(f"Sampling: mixed (all difficulties)")
    ...
    def sample_batch(bs):
        sizes = np.array([train_data[b]['size'] for b in all_buckets], dtype=np.int64)
        total = sizes.sum()
        probs = sizes / total
        counts = np.random.multinomial(bs, probs)
        x_parts = []
        t_parts = []
        for b, count in zip(all_buckets, counts):
            if count == 0:
                continue
    ...
    log(f"2D RoPE baseline (16 iters): 82.5%")

    final_path = os.path.join(output_dir, "model_32iters.pt")
