# New baseline: Train on 2.7M sudoku-extreme (matching Kaggle quantity)
# Uses no_x_after_init architecture, 70K steps, reverse curriculum
...
n_iterations = 16
lr = 1e-3
total_steps = 70000  # 70K steps (stabilizes by then)
batch_size = 512
sam_rho = 0.05
train_size = 2700000  # Match Kaggle quantity
...
print(f"\nBaseline: 2.7M sudoku-extreme, 70K steps")
print(f"Architecture: no_x_after_init")
print(f"BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_extreme_baseline.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Baseline: 2.7M sudoku-extreme, 70K steps")
log("="*60)
results = evaluate_all()
total_r = results.pop('_total')
...
log(f"Previous (400K, 100K steps): 63.4%")
log(f"TRM comparison: 87.4%")

torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_extreme_baseline.pt")
log("Model saved to model_extreme_baseline.pt")
