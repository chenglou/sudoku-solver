# Recurrence + No Preds: Concat variant without preds in input
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # CHANGED: input_proj takes (10 + d_model) instead of (10 + 9 + d_model)
        # No preds, just x and h_prev
        self.input_proj = nn.Linear(10 + d_model, d_model)
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            # CHANGED: concat x and h_prev only, no preds
...
print(f"\nRecurrence + No Preds (concat variant)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_concat_nopred.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence + No Preds (concat)")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_concat_nopred.pt")
log("Model saved to model_recur_concat_nopred.pt")
