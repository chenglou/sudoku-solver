# Experiment: 2D RoPE positional encoding (no sudoku pos encoding)
# Based on exp_faster.py but replaces row/col/box embeddings with 2D Rotary
# Position Embeddings applied to Q/K in attention. Uses row index for first
# half of head dims, col index for second half. Base frequency = 10 (not 10000)
# since positions only range 0-8.
...
CHECKPOINT_PREFIX = "faster_2drope_checkpoint_step"

CONFIG = {
    'experiment': 'exp_faster_2drope',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_faster_2drope.log"
...
# head_dim=32, split into 16 dims for row + 16 dims for col
# Each half has 8 frequency pairs
...
rope_base = 10.0  # small base for positions 0-8

_freqs = 1.0 / (rope_base ** (torch.arange(rope_pairs).float() * 2 / rope_half))
_row_angles = ROW_IDX.float().unsqueeze(1) * _freqs.unsqueeze(0)  # (81, 8)
_col_angles = COL_IDX.float().unsqueeze(1) * _freqs.unsqueeze(0)  # (81, 8)
_angles = torch.cat([_row_angles, _col_angles], dim=-1)  # (81, 16)
ROPE_COS = _angles.cos()  # (81, 16)
ROPE_SIN = _angles.sin()  # (81, 16)


def apply_rope(x, cos, sin):
    """Apply rotary embeddings. x: (B, n_heads, 81, head_dim)"""
    B, H, L, D = x.shape
    # Reshape into pairs: (B, H, 81, D//2, 2)
    pairs = x.reshape(B, H, L, D // 2, 2)
    x0, x1 = pairs[..., 0], pairs[..., 1]
    # cos, sin: (81, D//2) -> (1, 1, 81, D//2)
...
class RoPETransformerLayer(nn.Module):
    """TransformerEncoderLayer with 2D RoPE instead of positional embeddings.
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # No positional embeddings — RoPE handles position in attention
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: 2D RoPE (d_model={d_model}, head_dim={d_model // n_heads})")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_faster_2drope")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_faster_2drope.pt")
