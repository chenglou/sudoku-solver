# Experiment: ALiBi positional encoding (no sudoku pos encoding)
# Based on exp_faster.py but replaces row/col/box embeddings with ALiBi
# (Attention with Linear Biases). Fixed per-head linear decay based on
# 1D flattened distance. Fully sudoku-agnostic, zero learned pos params.
...
CHECKPOINT_PREFIX = "faster_alibi_checkpoint_step"

CONFIG = {
    'experiment': 'exp_faster_alibi',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
# Precompute ALiBi bias: fixed per-head linear decay on 1D distance
# Slopes: geometric sequence 2^(-8*h/H) for h=1..H
_slopes = torch.tensor([2.0 ** (-8.0 * (i + 1) / n_heads) for i in range(n_heads)])
_positions = torch.arange(81)
_distance = (_positions.unsqueeze(1) - _positions.unsqueeze(0)).abs().float()  # (81, 81)
# Shape: (n_heads, 81, 81) — broadcasts over batch dim in SDPA
ALIBI_BIAS = -_slopes.view(n_heads, 1, 1) * _distance.unsqueeze(0)
...
class ALiBiTransformerLayer(nn.Module):
    """TransformerEncoderLayer with ALiBi bias in attention.
    ...
    def forward(self, x, alibi_bias):
        ...
        # alibi_bias: (n_heads, 81, 81) broadcasts over batch dim
        attn_out = F.scaled_dot_product_attention(
            q, k, v, attn_mask=alibi_bias,
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # No positional embeddings — ALiBi handles position in attention
        self.layers = nn.ModuleList([
            ALiBiTransformerLayer(d_model, n_heads, d_ff)
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: ALiBi (d_model={d_model}, head_dim={d_model // n_heads})")
    print(f"Architecture: d_model={d_model}, d_ff={d_ff}, n_layers={n_layers}")
    print(f"ALiBi slopes: {[f'{s:.6f}' for s in _slopes.tolist()]}")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_faster_alibi")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_faster_alibi.pt")
