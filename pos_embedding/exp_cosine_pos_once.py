# Experiment: Cosine LR decay + pos_embed once at init
# Based on exp_cosine.py (84.0% SOTA)
# Changes:
#   - Add pos_embed to initial h_prev only, not every iteration
# Hypothesis: Position info may propagate through h_prev, making repeated addition redundant
...
CHECKPOINT_PREFIX = "cosine_pos_once_checkpoint_step"

CONFIG = {
    'experiment': 'exp_cosine_pos_once',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x, return_all=False):
        ...
        # ABLATION: Add pos_embed ONCE at init, not every iteration
        h_prev = self.initial_encoder(x) + pos_embed
        preds = torch.zeros(batch_size, 81, 9, device=device)

        all_logits = []
        for _ in range(n_iterations):
            # No pos_embed here - only h_prev and preds
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR + pos_embed ONCE at init")
    ...
    log_path = os.path.join(output_dir, "exp_cosine_pos_once.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR + pos_embed once")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Cosine baseline (pos every iter): 84.0%")

    final_path = os.path.join(output_dir, "model_cosine_pos_once.pt")
