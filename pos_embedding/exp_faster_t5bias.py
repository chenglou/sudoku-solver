# Experiment: T5-style relative position bias (no sudoku pos encoding)
# Based on exp_faster.py but replaces row/col/box embeddings with learned
# relative position bias added to attention logits. Fully sudoku-agnostic.
...
CHECKPOINT_PREFIX = "faster_t5bias_checkpoint_step"

CONFIG = {
    'experiment': 'exp_faster_t5bias',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_faster_t5bias.log"
...
# Relative position indices: rel[i,j] = i - j + 80, range [0, 160]
REL_POS_IDX = torch.arange(81).unsqueeze(1) - torch.arange(81).unsqueeze(0) + 80
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # T5-style: learned scalar bias per relative distance, shared across heads
        self.rel_pos_bias = nn.Embedding(161, 1)  # 161 = 2*80+1
    ...
    def forward(self, x, return_all=False):
        ...
        # Compute attention bias: (81, 81)
        rel_idx = REL_POS_IDX.to(device)
        attn_bias = self.rel_pos_bias(rel_idx).squeeze(-1)  # (81, 81)
        ...
        for _ in range(n_iterations):
            ...
            h = self.transformer(h, mask=attn_bias)
            h_prev = h
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: T5-style rel pos bias (d_model={d_model}, head_dim={d_model // n_heads})")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_faster_t5bias")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_faster_t5bias.pt")
