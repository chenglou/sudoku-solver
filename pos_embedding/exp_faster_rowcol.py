# Experiment: Row+Col positional embedding only (no box)
# Based on exp_faster.py but removes the box embedding.
# Tests whether the 3x3 box structure is the key sudoku-specific bias.
...
CHECKPOINT_PREFIX = "faster_rowcol_checkpoint_step"

CONFIG = {
    'experiment': 'exp_faster_rowcol',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
...
log_name = "exp_faster_rowcol.log"
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # No box_embed — just 2D grid coordinates
    ...
    def forward(self, x, return_all=False):
        ...
        pos_embed = self.row_embed(row_idx) + self.col_embed(col_idx)
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Row+Col only (d_model={d_model}, head_dim={d_model // n_heads})")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_faster_rowcol")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_faster_rowcol.pt")
