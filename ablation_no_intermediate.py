# ABLATION: No intermediate supervision - only final iteration loss
# Keep: iterative refinement (16 iter), sudoku pos encoding
import torch
...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return logits  # Only return final logits
...
log_file = open("ablation_no_intermediate.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
for step in range(steps):
    ...
    logits = model(x_batch)  # Only final logits
    ...
    # ABLATION: Only final loss, no intermediate supervision
    logits_holes = logits[hole_b, hole_c]
