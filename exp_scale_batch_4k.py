# Experiment: Scale batch size to 4096 (8x baseline)
# Baseline: d=128, L=4, BS=512, lr=1e-3 -> 71.4%
# Scale batch: BS=2048, lr=1e-3 -> 73.7%
# This: BS=4096, lr=1.5e-3
...
CHECKPOINT_PREFIX = "scale_batch_4k_checkpoint_step"

# Config for checkpoint verification
CONFIG = {
    'experiment': 'exp_scale_batch_4k',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
n_layers = 4
n_iterations = 16
lr = 1.5e-3  # Slightly scaled up from 1e-3
total_steps = 70000
batch_size = 4096  # 8x baseline, 2x scale_batch
sam_rho = 0.05
train_size = 2700000
...
def train(output_dir="."):
    ...
    log_path = os.path.join(output_dir, "exp_scale_batch_4k.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Scale BATCH SIZE 4K")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Scale batch (BS=2048): 73.7%")
    log(f"TRM: 87.4%")

    final_path = os.path.join(output_dir, "model_scale_batch_4k.pt")
