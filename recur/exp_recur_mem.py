# Recurrence Experiment 4: Separate memory bank
# mem = mem + mem_update(h)  # accumulate into memory
# h uses mem as additional input
...
# Hyperparameters
d_model = 128
d_mem = 64  # NEW: memory dimension (smaller than d_model)
n_heads = 4
d_ff = 512
n_layers = 4
n_iterations = 16
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # NEW: Memory projections
        self.mem_proj = nn.Linear(d_mem, d_model)  # project memory to add to h
        self.mem_update = nn.Linear(d_model, d_mem)  # update memory from h
    ...
    def forward(self, x, return_all=False):
        ...
        mem = torch.zeros(batch_size, 81, d_mem, device=device)  # NEW: separate memory bank
        all_logits = []
        for _ in range(n_iterations):
            x_in = torch.cat([x, preds], dim=-1)
            h = self.input_proj(x_in)
            h = h + pos_embed + self.mem_proj(mem)  # NEW: add projected memory
            h = self.transformer(h)
            mem = mem + self.mem_update(h)  # NEW: accumulate into memory
...
print(f"\nRecurrence Exp 4: Separate Memory Bank")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_mem.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence Exp 4: Separate Memory Bank")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_mem.pt")
log("Model saved to model_recur_mem.pt")
