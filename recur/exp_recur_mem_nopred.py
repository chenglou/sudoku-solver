# Recurrence + No Preds: Memory bank variant without preds in input
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # CHANGED: input_proj only takes puzzle (10 dims), not preds
        ...
        # Memory projections
        self.mem_proj = nn.Linear(d_mem, d_model)
        self.mem_update = nn.Linear(d_model, d_mem)
    ...
    def forward(self, x, return_all=False):
        ...
        mem = torch.zeros(batch_size, 81, d_mem, device=device)
        all_logits = []
        for _ in range(n_iterations):
            # CHANGED: only use x, not concat(x, preds)
            h = self.input_proj(x)
            h = h + pos_embed + self.mem_proj(mem)
            h = self.transformer(h)
            mem = mem + self.mem_update(h)
...
print(f"\nRecurrence + No Preds (mem variant)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_mem_nopred.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence + No Preds (mem)")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_mem_nopred.pt")
log("Model saved to model_recur_mem_nopred.pt")
