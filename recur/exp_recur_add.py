# Recurrence Experiment 1: Add previous hidden state to input
# h = transformer(input_proj(concat(x, preds)) + h_prev)
...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            ...
            h = h + pos_embed + h_prev  # NEW: add previous hidden state
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence Exp 1: Add h_prev")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_add.pt")
log("Model saved to model_recur_add.pt")
