# Experiment: Cosine LR decay
# Based on exp_warmup.py (78.5% baseline)
# Changes:
#   - Cosine LR decay after warmup (lr -> lr * 0.01)
# Hypothesis: Gradual LR decay may help convergence
...
CHECKPOINT_PREFIX = "cosine_checkpoint_step"

# Config for checkpoint verification
CONFIG = {
    'experiment': 'exp_cosine',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR decay (min_ratio={lr_min_ratio})")
    ...
    log_path = os.path.join(output_dir, "exp_cosine.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR decay")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_cosine.pt")
