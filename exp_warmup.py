...
# Based on exp_scale_batch_4k.py (76.3% baseline)
# Changes:
#   - LR warmup for 2000 steps (linear ramp from 0 to lr)
# Note: weight_decay=0.01 is already AdamW default in baseline
# Hypothesis: warmup helps early training stability
...
CHECKPOINT_PREFIX = "warmup_checkpoint_step"

# Config for checkpoint verification
CONFIG = {
    'experiment': 'exp_warmup',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
n_iterations = 16
lr = 1.5e-3
warmup_steps = 2000  # NEW: warmup for stability
# Note: weight_decay=0.01 is AdamW default, same as baseline
total_steps = 70000
batch_size = 4096
sam_rho = 0.05
train_size = 2700000
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: LR warmup (weight_decay=0.01 default, same as baseline)")
    ...
    log_path = os.path.join(output_dir, "exp_warmup.log")
    ...
    log(f"Baseline (no warmup): 76.3%")
    log(f"nano-trm: 87.4%")

    final_path = os.path.join(output_dir, "model_warmup.pt")
