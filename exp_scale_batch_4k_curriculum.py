# Experiment: BS=4096 with REGULAR curriculum (easy->hard) for comparison
# Same setup as v2 but phases reversed: easy first, then progressively harder
...
CHECKPOINT_PREFIX = "scale_batch_4k_curriculum_checkpoint_step"

CONFIG = {
    'experiment': 'exp_scale_batch_4k_curriculum',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
# REGULAR curriculum: easy -> hard (opposite of reverse)
# Rating 0 = easiest, Rating 51+ = hardest
PHASES = [
    (0, 2000, 0, 2, "Phase 1: Easy only (rating 0-2)"),
    (2000, 4000, 0, 10, "Phase 2: Easy-Med (rating 0-10)"),
    (4000, 6000, 0, 50, "Phase 3: Easy-Hard (rating 0-50)"),
    (6000, 10000, 0, 1000, "Phase 4: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    # Build phase buckets for REGULAR curriculum (filter by max rating)
    phase_buckets = {}
    for start, end, min_rating, max_rating, name in PHASES:
        buckets_for_phase = [k for k in train_data.keys() if k[1] <= max_rating]
        total = sum(train_data[k]['size'] for k in buckets_for_phase)
        phase_buckets[max_rating] = buckets_for_phase
    ...
    print(f"\nExperiment: BS=4096 with REGULAR curriculum (easy->hard)")
    print(f"Phases: 0-2K (easy) -> 2K-4K (easy-med) -> 4K-6K (easy-hard) -> 6K-10K (all)")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_scale_batch_4k_curriculum.log")
    ...
    def get_phase(step):
        for start, end, min_rating, max_rating, name in PHASES:
            if start <= step < end:
                return phase_buckets[max_rating], name
        return None, None
    ...
    def evaluate_all():
        ...
        with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
            for name, data in test_data.items():
                x_test, puzzles, solutions = data['x'], data['puzzles'], data['solutions']
                puzzles_solved = 0
    ...
    current_phase_name, current_buckets = None, None
    eval_interval = 1000
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - BS=4096 with REGULAR curriculum (easy->hard)")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_scale_batch_4k_curriculum.pt")
