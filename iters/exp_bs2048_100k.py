# BS=2048, 16 iters, reverse curriculum, 100K steps (2x baseline).
# Purpose: test whether more training steps improves accuracy.
# Same architecture and curriculum phases, just train longer.
# Control: exp_bs2048_baseline (50K steps → 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "bs2048_100k_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_100k',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048, 100K steps (2x baseline training)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_100k")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"BS=2048 baseline (50K steps): 81.4%")

    final_path = os.path.join(output_dir, "model_bs2048_100k.pt")
