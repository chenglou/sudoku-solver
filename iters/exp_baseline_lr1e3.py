# Baseline architecture with LR=1e-3 (vs baseline LR=1.5e-3).
# Purpose: test if LR controls iteration scaling stability.
# Control: exp_bs2048_baseline (LR=1.5e-3, 81.4% at 16, 98.1% at 1024).
...
CHECKPOINT_PREFIX = "baseline_lr1e3_checkpoint_step"

CONFIG = {
    'experiment': 'exp_baseline_lr1e3',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_baseline_lr1e3.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Baseline with LR=1e-3 (vs 1.5e-3)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_baseline_lr1e3")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_baseline_lr1e3.pt")
