# 3-phase curriculum: Hard → Easy+ → All (drop Medium+ phase), 40K total steps.
# Purpose: test if the Medium+ phase is redundant, keeping phase durations constant.
# Each phase keeps its original duration (10K/10K/20K), just removing Medium+ entirely.
# Control: exp_bs2048_baseline (4-phase, 50K steps → 81.4% at 16 iters).
...
CHECKPOINT_PREFIX = "3phase_40k_checkpoint_step"

CONFIG = {
    'experiment': 'exp_3phase_40k',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_3phase_40k.log"

# 3 phases: Hard → Easy+ → All (skip Medium+)
# Keep original phase durations: 10K hard, 10K easy+, 20K all
PHASES = [
    (0, 10000, 21, "Phase 1: Hard only (rating 21+)"),
    (10000, 20000, 1, "Phase 2: Easy+ (rating 1+)"),
    (20000, 40000, 0, "Phase 3: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: 3-phase curriculum (Hard → Easy+ → All), 40K steps")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_3phase_40k")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"4-phase baseline (50K): 81.4%")

    final_path = os.path.join(output_dir, "model_3phase_40k.pt")
