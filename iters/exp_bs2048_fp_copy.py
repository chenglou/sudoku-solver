# BS=2048, 16 iters, reverse curriculum + self-consistency copy loss.
# Purpose: for cells correct at iteration t-1, add MSE loss between
# logits[t] and logits[t-1].detach(). Teaches "don't change what you got right"
# without using ground truth — purely self-referential.
# Control: exp_bs2048_baseline (same without copy loss → 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "bs2048_fp_copy_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_fp_copy',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_fp_copy.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 + self-consistency copy loss on correct cells")
    ...
    def compute_loss(x_batch, t_batch):
        ...
        for i, logits in enumerate(all_logits):
            ...
            if i > 0:
                prev_correct = (all_logits[i-1].detach().argmax(dim=-1) == t_batch).float() * mask
                copy_loss = ((F.softmax(logits, dim=-1) - F.softmax(all_logits[i-1].detach(), dim=-1)) ** 2).sum(dim=-1)
                loss = loss + (copy_loss * prev_correct).sum() / prev_correct.sum().clamp(min=1)
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_fp_copy")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"BS=2048 baseline: 81.4%")

    final_path = os.path.join(output_dir, "model_bs2048_fp_copy.pt")
