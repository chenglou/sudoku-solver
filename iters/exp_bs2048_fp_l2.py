# BS=2048, 16 iters, reverse curriculum + L2 fixed-point loss.
# Purpose: for cells correct at iteration t-1, add MSE loss between
# softmax(logits[t]) and one-hot target. Penalizes deviation from correct answer.
# Control: exp_bs2048_baseline (same without L2 loss → 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "bs2048_fp_l2_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_fp_l2',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 + L2 fixed-point loss on correct cells")
    ...
    def compute_loss(x_batch, t_batch):
        ...
        target_onehot = F.one_hot(t_batch, 9).float()
        loss = 0
        for i, logits in enumerate(all_logits):
            ...
            if i > 0:
                ...
                l2 = ((F.softmax(logits, dim=-1) - target_onehot) ** 2).sum(dim=-1)
                loss = loss + (l2 * prev_correct).sum() / prev_correct.sum().clamp(min=1)
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_fp_l2")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_bs2048_fp_l2.pt")
