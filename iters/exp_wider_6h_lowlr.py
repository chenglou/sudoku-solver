# Wider model with lower LR: d_model=192, d_ff=768, n_heads=6 (head_dim=32).
# Purpose: test if wider model's iteration collapse was caused by too-high LR.
# LR=1e-3 (vs 1.5e-3 in exp_wider_6h which collapsed at 64 iters).
# Control: exp_wider_6h (LR=1.5e-3, 84.7% at 16 but 1.1% at 1024).
...
CHECKPOINT_PREFIX = "wider_6h_lowlr_checkpoint_step"

CONFIG = {
    'experiment': 'exp_wider_6h_lowlr',
    'd_model': 192,
    'd_ff': 768,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_wider_6h_lowlr.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Wider model + lower LR (d_model=192, n_heads=6, LR=1e-3)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_wider_6h_lowlr")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Wider LR=1.5e-3: 84.7% at 16, collapsed at 64")

    final_path = os.path.join(output_dir, "model_wider_6h_lowlr.pt")
