# BS=2048, 16 iters, reverse curriculum + fixed-point preservation loss.
# Purpose: teach f to preserve cells that are already correct.
# At each iteration t > 0, cells that were correct at t-1 get 2x CE weight.
# Control: exp_bs2048_baseline (same without preservation loss → 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "bs2048_fixedpoint_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_fixedpoint',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_fixedpoint.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 with fixed-point preservation loss (2x weight on previously-correct cells)")
    ...
    def compute_loss(x_batch, t_batch):
        ...
        for i, logits in enumerate(all_logits):
            ...
            if i > 0:
                prev_correct = (all_logits[i-1].detach().argmax(dim=-1) == t_batch).float()
                per_cell = per_cell * (1.0 + prev_correct)
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_fixedpoint")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_bs2048_fixedpoint.pt")
