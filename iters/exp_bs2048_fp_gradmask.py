# BS=2048, 16 iters, reverse curriculum + gradient masking.
# Purpose: zero out CE loss for cells correct at iteration t-1.
# Prevents the model from learning to change cells it already got right.
# Control: exp_bs2048_baseline (same without masking → 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "bs2048_fp_gradmask_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_fp_gradmask',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_bs2048_fp_gradmask.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 + gradient masking on correct cells")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_fp_gradmask")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_bs2048_fp_gradmask.pt")
