# Wider model: d_model=192, d_ff=768, n_heads=6 (head_dim=32).
# Purpose: test whether more capacity improves accuracy and iteration scaling.
# ~1.7M params vs baseline's ~800K. Everything else identical.
# Control: exp_bs2048_baseline (d_model=128, 98.1% at 1024 iters).
...
CHECKPOINT_PREFIX = "wider_6h_checkpoint_step"

CONFIG = {
    'experiment': 'exp_wider_6h',
    'd_model': 192,
    'd_ff': 768,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Wider model (d_model=192, n_heads=6, d_ff=768)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_wider_6h")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"BS=2048 baseline (d_model=128): 81.4%")

    final_path = os.path.join(output_dir, "model_wider_6h.pt")
