# Wider model with LR=2e-3: d_model=192, d_ff=768, n_heads=6.
# Purpose: test if higher LR fixes wider model's iteration collapse.
# d=192 collapsed at LR=1.5e-3 (64 iters) and LR=1e-3 (256 iters).
# LR=2e-3 was optimal for d=128 (98.9% at 1024).
# Control: exp_wider_6h (LR=1.5e-3), exp_wider_6h_lowlr (LR=1e-3).
...
CHECKPOINT_PREFIX = "wider_6h_lr2e3_checkpoint_step"

CONFIG = {
    'experiment': 'exp_wider_6h_lr2e3',
    'd_model': 192,
    'd_ff': 768,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_wider_6h_lr2e3.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Wider model with LR=2e-3 (d_model=192, n_heads=6, d_ff=768)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_wider_6h_lr2e3")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Wider LR=1.5e-3: 84.7% at 16, collapses at 64. LR=1e-3: collapses at 256.")

    final_path = os.path.join(output_dir, "model_wider_6h_lr2e3.pt")
