# Baseline architecture with LR=3e-3.
# Purpose: test if pushing LR even higher continues to improve iteration scaling.
# Control: exp_baseline_lr3e3 (LR=2e-3, 81.8% at 16, 98.9% at 1024).
...
CHECKPOINT_PREFIX = "baseline_lr3e3_checkpoint_step"

CONFIG = {
    'experiment': 'exp_baseline_lr3e3',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_baseline_lr3e3.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Baseline with LR=3e-3")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_baseline_lr3e3")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    final_path = os.path.join(output_dir, "model_baseline_lr3e3.pt")
