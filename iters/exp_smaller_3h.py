# Smaller model: d_model=96, d_ff=384, n_heads=3 (head_dim=32).
# Purpose: test if smaller model has better iteration scaling.
# ~450K params vs baseline's ~800K. If iteration scaling improves with
# smaller models, it suggests the baseline might be too large.
# Control: exp_bs2048_baseline (d_model=128, 81.4% at 16, 98.1% at 1024).
...
CHECKPOINT_PREFIX = "smaller_3h_checkpoint_step"

CONFIG = {
    'experiment': 'exp_smaller_3h',
    'd_model': 96,
    'd_ff': 384,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Smaller model (d_model=96, n_heads=3, d_ff=384)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_smaller_3h")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Baseline (d_model=128): 81.4%")

    final_path = os.path.join(output_dir, "model_smaller_3h.pt")
