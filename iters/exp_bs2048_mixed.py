# BS=2048, 16 iters, mixed sampling (no curriculum).
# Purpose: isolate the effect of reverse curriculum vs mixed sampling.
# Control: exp_bs2048_baseline (same but with reverse curriculum → 81.4%).
...
CHECKPOINT_PREFIX = "bs2048_mixed_checkpoint_step"

CONFIG = {
    'experiment': 'exp_bs2048_mixed',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=2048 mixed sampling (no curriculum)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_bs2048_mixed")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"BS=2048 curriculum baseline: 81.4%")

    final_path = os.path.join(output_dir, "model_bs2048_mixed.pt")
