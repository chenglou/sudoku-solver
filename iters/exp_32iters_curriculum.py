# BS=2048, 32 iters, reverse curriculum.
# Purpose: isolate the effect of 32 training iters from sampling strategy.
# Control: exp_32iters (same but with mixed sampling → 83.0%).
# Also compare to exp_bs2048_baseline (16 iters + curriculum → 81.4%).
...
CHECKPOINT_PREFIX = "32iters_curriculum_checkpoint_step"

CONFIG = {
    'experiment': 'exp_32iters_curriculum',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_32iters_curriculum.log"
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: 32 iterations with reverse curriculum")
    print(f"Architecture: d_model={d_model}, d_ff={d_ff}, n_layers={n_layers}")
    print(f"Iterations: {n_iterations}")
    print(f"Sampling: reverse curriculum (hard → easy)")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_32iters_curriculum")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"32-iter mixed baseline: 83.0%")

    final_path = os.path.join(output_dir, "model_32iters_curriculum.pt")
