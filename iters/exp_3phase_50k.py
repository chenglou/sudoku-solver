# 3-phase curriculum: Hard → Easy+ → All (drop Medium+ phase), 50K total steps.
# Purpose: test if the Medium+ phase is redundant.
# Control: exp_bs2048_baseline (4-phase, same 50K steps → 81.4% at 16 iters).
...
CHECKPOINT_PREFIX = "3phase_50k_checkpoint_step"

CONFIG = {
    'experiment': 'exp_3phase_50k',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 2048,
...
log_name = "exp_3phase_50k.log"

# 3 phases: Hard → Easy+ → All (skip Medium+)
# Redistribute: 30% hard, 30% easy+, 40% all
PHASES = [
    (0, 15000, 21, "Phase 1: Hard only (rating 21+)"),
    (15000, 30000, 1, "Phase 2: Easy+ (rating 1+)"),
    (30000, 50000, 0, "Phase 3: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: 3-phase curriculum (Hard → Easy+ → All), 50K steps")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - exp_3phase_50k")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"4-phase baseline: 81.4%")

    final_path = os.path.join(output_dir, "model_3phase_50k.pt")
