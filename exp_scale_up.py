# Experiment: Scale UP the model to ~5M params (matching TRM)
# Larger dimensions to test if model size is the bottleneck
...
# Hyperparameters - SCALED UP to ~5M params
d_model = 256      # was 128
n_heads = 8        # was 4
d_ff = 1024        # was 512
n_layers = 8       # was 4
...
lr = 1e-3
total_steps = 70000
micro_batch_size = 128  # fits in memory
accum_steps = 4         # gradient accumulation
batch_size = micro_batch_size * accum_steps  # effective BS=512
sam_rho = 0.05
train_size = 2700000
...
print(f"\nExperiment: Scale UP (targeting TRM size)")
print(f"Architecture: d_model={d_model}, n_layers={n_layers}, n_heads={n_heads}, d_ff={d_ff}")
print(f"Iterations: {n_iterations}, micro_BS={micro_batch_size}, accum={accum_steps}, effective_BS={batch_size}")
print(f"SAM rho={sam_rho}")

log_file = open("exp_scale_up.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
for step in range(total_steps):
    ...
    # Sample full batch, split into micro-batches
    micro_batches = []
    for _ in range(accum_steps):
        x_batch, holes_batch, counts_batch = sample_batch(current_buckets, micro_batch_size)
        micro_batches.append((x_batch, holes_batch, counts_batch))

    # SAM first step: accumulate gradients over micro-batches
    optimizer.zero_grad()
    for x_batch, holes_batch, counts_batch in micro_batches:
        with torch.autocast('cuda', dtype=torch.bfloat16):
            loss1, _, _, _, _ = compute_loss(x_batch, holes_batch, counts_batch)
        (loss1 / accum_steps).backward()
    optimizer.first_step()

    # SAM second step: accumulate gradients over same micro-batches
    optimizer.zero_grad()
    total_loss2 = 0
    for i, (x_batch, holes_batch, counts_batch) in enumerate(micro_batches):
        with torch.autocast('cuda', dtype=torch.bfloat16):
            loss2, all_logits, hole_b, hole_c, targets = compute_loss(x_batch, holes_batch, counts_batch)
        (loss2 / accum_steps).backward()
        total_loss2 += loss2.item()
    optimizer.second_step()
    loss2_avg = total_loss2 / accum_steps

    if step % 100 == 0 or step == total_steps - 1:
        with torch.no_grad():
            # Use last micro-batch for accuracy estimate
        ...
        if step % 5000 == 0 or step == total_steps - 1:
            ...
            log(f"Step {step:5d} | Loss: {loss2_avg:.4f} Acc: {train_acc:.2%} | " +
...
log(f"TRM (5M params): 87.4%")

torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_scale_up.pt")
log("Model saved to model_scale_up.pt")
