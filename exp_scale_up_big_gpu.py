...
# Modal-agnostic - can run locally or via modal_run.py
#
# Local:  python exp_scale_up_big_gpu.py
# Modal:  modal run modal_run.py
...
def train(output_dir="."):
    ...
    log_path = os.path.join(output_dir, "exp_scale_up_big_gpu.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Scale UP with TRUE batch size")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Previous scale-up (grad accum): 69.7%")
    log(f"TRM (5M params): 87.4%")

    final_model_path = os.path.join(output_dir, "model_scale_up_big_gpu.pt")
