# Ablation: Encode x once, then iterate without re-feeding x (TRM-style)
# Tests whether constantly re-feeding x is necessary or a crutch
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # Pred projection for feeding preds into iteration (no x)
...
print(f"\nAblation: No x after init (encode once)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_no_x_after_init.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Ablation: No x after init")
log("="*60)
results = evaluate_all()
...
log(f"Baseline (with x every iter): 2265/2500")

torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_no_x_after_init.pt")
log("Model saved to model_no_x_after_init.pt")
