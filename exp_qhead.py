# Experiment: Q-head (learned confidence/halt signal)
# Based on exp_faster_2drope.py (2D RoPE baseline).
# Adds a Q-head that predicts per-iteration whether the puzzle is fully solved.
# Trained with BCE loss (target = all empty cells correct).
# Q-head: Linear(d_model, 1) on mean-pooled hidden state.
# Init: weight=0, bias=-5 (default: "not done yet").
...
CHECKPOINT_PREFIX = "qhead_checkpoint_step"

CONFIG = {
    'experiment': 'exp_qhead',
    'd_model': 128,
    'd_ff': 512,
    'n_layers': 4,
    'batch_size': 4096,
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            ...
            if return_all:
                ...
                # Q-head on mean-pooled hidden state
...
def train(output_dir="."):
        ...
        with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
            for name, data in test_data.items():
                ...
                for start in range(0, len(puzzles), 256):
                    ...
                    all_logits, all_q_logits = model(batch_x, return_all=True)
                    logits = all_logits[-1]
                    preds_full = logits.argmax(dim=-1).cpu()

                    # Q-head accuracy at final iteration
                    ...
                    for b, (puzzle, solution) in enumerate(zip(puzzles[start:end], solutions[start:end])):
                        ...
                        if is_solved:
                            puzzles_solved += 1
                        # Q-head accuracy: does it correctly predict solved/unsolved?
    ...
    log(f"2D RoPE baseline: 82.5%")

    final_path = os.path.join(output_dir, "model_qhead.pt")
