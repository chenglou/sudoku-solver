# ABLATION: No iteration - single forward pass (n_iterations=1)
# Keep: sudoku pos encoding
# Note: With n_iterations=1, intermediate supervision is N/A (only one iteration)
import torch
...
d_ff = 512
n_layers = 4
n_iterations = 1  # ABLATION: No iteration, single forward pass
lr = 1e-3
steps = 100000
n_train = 100000
n_test = 1000
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # With n_iterations=1, no need for prediction feedback, but keep same input size for fair comparison
    ...
    def forward(self, x):
        ...
        # ABLATION: Only one iteration, no feedback loop
...
print(f"\nABLATION: No iteration (n_iterations=1)")
print(f"Training on {device}...")

log_file = open("ablation_no_iteration.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
