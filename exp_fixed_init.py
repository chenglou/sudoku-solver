# Experiment: Fixed random hidden state init (nano-trm style)
# Based on exp_warmup.py (78.5% baseline with warmup)
# Changes:
#   - Replace learned initial_encoder with fixed random nn.Buffer (std=1.0)
#   - Add puzzle_proj inside the loop to feed puzzle info
# Hypothesis: Fixed init forces model to be more robust, better gradient flow
...
CHECKPOINT_PREFIX = "fixed_init_checkpoint_step"

# Config for checkpoint verification
CONFIG = {
    'experiment': 'exp_fixed_init',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
lr = 1.5e-3
warmup_steps = 2000
h_init_std = 1.0  # NEW: nano-trm uses std=1.0 for hidden state init
total_steps = 70000
batch_size = 4096
sam_rho = 0.05
train_size = 2700000
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # NEW: Fixed random hidden state init (not learned)
        # nano-trm uses nn.Buffer with std=1.0
        self.register_buffer('h_init', torch.randn(1, 81, d_model) * h_init_std)

        # NEW: Puzzle projection added inside loop (since h_init doesn't encode puzzle)
        self.puzzle_proj = nn.Linear(10, d_model)
    ...
    def forward(self, x, return_all=False):
        ...
        # NEW: h_prev starts as fixed random (same for all puzzles)
        h_prev = self.h_init.expand(batch_size, -1, -1)

        # NEW: Puzzle encoding added inside loop
        puzzle_embed = self.puzzle_proj(x)

        preds = torch.zeros(batch_size, 81, 9, device=device)

        all_logits = []
        for _ in range(n_iterations):
            # NEW: Add puzzle_embed here since h_prev doesn't have puzzle info
            h = h_prev + puzzle_embed + self.pred_proj(preds) + pos_embed
...
def train(output_dir="."):
    ...
    print(f"Model buffers (fixed): {buffer_count:,} (h_init)")
    ...
    print(f"\nExperiment: Fixed random hidden state init (std={h_init_std})")
    ...
    log_path = os.path.join(output_dir, "exp_fixed_init.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Fixed Random Init")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Baseline with warmup: 78.5%")
    log(f"nano-trm: 87.4%")

    final_path = os.path.join(output_dir, "model_fixed_init.pt")
