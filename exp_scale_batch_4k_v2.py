# Experiment: BS=4096 with properly scaled curriculum phases
# Original (BS=512): 70K steps, phases at 14K/28K/42K/70K
# This (BS=4096): ~10K steps, phases scaled to same data seen
#
# Data equivalence: 70K * 512 = 35.84M samples = 8750 * 4096
# Using 10K steps for slightly more training
...
CHECKPOINT_PREFIX = "scale_batch_4k_v2_checkpoint_step"

CONFIG = {
    'experiment': 'exp_scale_batch_4k_v2',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
n_iterations = 16
lr = 1.5e-3
total_steps = 10000  # Scaled down from 70K (same data as BS=512 @ 70K)
batch_size = 4096
sam_rho = 0.05
train_size = 2700000

# Scaled reverse curriculum phases (same data per phase as original)
# Original BS=512: 0-14K, 14K-28K, 28K-42K, 42K-70K
# Scaled BS=4096: 0-2K, 2K-4K, 4K-6K, 6K-10K
PHASES = [
    (0, 2000, 21, "Phase 1: Hard only (rating 21+)"),
    (2000, 4000, 6, "Phase 2: Medium+ (rating 6+)"),
    (4000, 6000, 1, "Phase 3: Easy+ (rating 1+)"),
    (6000, 10000, 0, "Phase 4: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: BS=4096 with SCALED curriculum phases")
    print(f"Architecture: d_model={d_model}, n_layers={n_layers}")
    print(f"Batch size: {batch_size}, lr: {lr}, total_steps: {total_steps}")
    print(f"Phases: 0-2K (hard) -> 2K-4K (med+) -> 4K-6K (easy+) -> 6K-10K (all)")
    print(f"Output directory: {output_dir}")

    log_path = os.path.join(output_dir, "exp_scale_batch_4k_v2.log")
    ...
    # Eval every 1K steps (10 evals total for 10K steps)
    eval_interval = 1000
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - BS=4096 with scaled curriculum")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Baseline (BS=512, 70K steps): 71.4%")
    log(f"BS=2048 (70K steps): 73.7%")
    log(f"BS=4096 unscaled (70K steps): running...")
    log(f"TRM: 87.4%")

    final_path = os.path.join(output_dir, "model_scale_batch_4k_v2.pt")
