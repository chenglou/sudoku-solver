# Recurrence Experiment 3: Gated update (GRU-style)
# gate = sigmoid(gate_proj(concat(h_prev, h_candidate)))
# h = gate * h_prev + (1 - gate) * h_candidate
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # NEW: Gate projection for GRU-style update
    ...
    def forward(self, x, return_all=False):
        ...
        h_prev = torch.zeros(batch_size, 81, d_model, device=device)  # NEW: previous hidden state
        all_logits = []
        for _ in range(n_iterations):
            x_in = torch.cat([x, preds], dim=-1)
            h_candidate = self.input_proj(x_in)
            h_candidate = h_candidate + pos_embed
            h_candidate = self.transformer(h_candidate)
            # NEW: GRU-style gated update
            gate = torch.sigmoid(self.gate_proj(torch.cat([h_prev, h_candidate], dim=-1)))
            h = gate * h_prev + (1 - gate) * h_candidate
            h_prev = h  # Save for next iteration
...
print(f"\nRecurrence Exp 3: Gated (GRU-style)")
print(f"Total steps: {total_steps}, BS={batch_size}, SAM rho={sam_rho}")

log_file = open("exp_recur_gated.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Recurrence Exp 3: Gated (GRU-style)")
log("="*60)
results = evaluate_all()
...
torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_recur_gated.pt")
log("Model saved to model_recur_gated.pt")
