...
# Predictions have their own update path with RMS normalization
# No explicit preds fed into h - preds updated separately
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # Initial encoder: x (10-dim) -> h (d_model)
        ...
        # Output head for h -> logits
        self.output_head = nn.Linear(d_model, 9)

        # Separate prediction update path (TRM-style)
        # Takes current h and pred_state, outputs delta for pred_state
        self.pred_update = nn.Sequential(
            nn.Linear(d_model + 9, d_ff),
            nn.GELU(),
            nn.Linear(d_ff, 9),
        )

        # RMS normalization for prediction residuals (key TRM insight)
        self.pred_norm = RMSNorm(9)
    ...
    def forward(self, x, return_all=False):
        ...
        h_prev = self.initial_encoder(x)  # (batch, 81, d_model)

        # Initialize prediction state (in logit space, not probability)
        pred_state = torch.zeros(batch_size, 81, 9, device=device)

        all_logits = []
        for _ in range(n_iterations):
            # Update h (no preds input - h is independent)
            h = h_prev + pos_embed
            h = self.transformer(h)
            h_prev = h

            # Update pred_state separately using h and current pred_state
            pred_input = torch.cat([h, pred_state], dim=-1)
            delta = self.pred_update(pred_input)

            # Residual + RMS norm (TRM-style)
            pred_state = self.pred_norm(pred_state + delta)

            # Output logits combines h projection and pred_state
            h_logits = self.output_head(h)
            logits = h_logits + pred_state
...
log_file = open("exp_norm_pred.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
...
log("\n" + "="*60)
log("FINAL RESULTS - Normalized prediction residuals")
log("="*60)
results = evaluate_all()
...
log(f"Baseline (no_x_after_init): 2248/2500")

torch.save({k.replace('_orig_mod.', ''): v for k, v in model.state_dict().items()}, "model_norm_pred.pt")
log("Model saved to model_norm_pred.pt")
