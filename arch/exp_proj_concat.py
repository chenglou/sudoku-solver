# Experiment: Project-then-concat input combination
# Change: concat(proj_digit(digit), proj_pred(preds), empty_embed(is_empty)) instead of adding
# Projects to smaller dims (64+48+16=128) to keep same d_model
import torch
...
# Projection dimensions for concat (must sum to d_model)
d_digit = 64
d_pred = 48
d_empty = 16
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        super().__init__()
        # EXPERIMENT: Project-then-concat (smaller projections that sum to d_model)
        self.proj_digit = nn.Linear(9, d_digit)
        self.proj_pred = nn.Linear(9, d_pred)
        self.empty_embed = nn.Embedding(2, d_empty)
    ...
    def forward(self, x, return_all=False):
        ...
        for _ in range(n_iterations):
            is_empty = x[:, :, 0].long()
            digit_onehot = x[:, :, 1:]
            # EXPERIMENT: Concat instead of add
            h = torch.cat([
                self.proj_digit(digit_onehot),
                self.proj_pred(preds),
                self.empty_embed(is_empty)
            ], dim=-1)
            h = h + pos_embed
...
log_file = open("exp_proj_concat.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
