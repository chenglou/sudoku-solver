# Experiment: 2 transformers (2 layers each), T1 handles iters 1-8, T2 handles iters 9-16
# Tests: early vs late iteration specialization
# Same ~800k params as sudoku.py, but 32 layer passes instead of 64
import torch
...
n_heads = 4
d_ff = 512
n_layers_per_transformer = 2  # 2 layers each (vs 4 in original)
n_transformers = 2
iters_per_transformer = 8     # 8 iterations each, total 16
lr = 1e-3
steps = 100000
n_train = 100000
n_test = 1000
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # Two separate transformers with 2 layers each
        encoder_layer1 = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=n_heads, dim_feedforward=d_ff,
            batch_first=True, norm_first=True,
        )
        self.transformer1 = nn.TransformerEncoder(encoder_layer1, num_layers=n_layers_per_transformer)

        encoder_layer2 = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=n_heads, dim_feedforward=d_ff,
            batch_first=True, norm_first=True,
        )
        self.transformer2 = nn.TransformerEncoder(encoder_layer2, num_layers=n_layers_per_transformer)
    ...
    def forward(self, x, return_all=False):
        ...
        # First 8 iterations with transformer1
        for _ in range(iters_per_transformer):
        ...
        # Next 8 iterations with transformer2
        for _ in range(iters_per_transformer):
            x_in = torch.cat([x, preds], dim=-1)
            h = self.input_proj(x_in)
            h = h + pos_embed
            h = self.transformer2(h)
            logits = self.output_head(h)
            preds = F.softmax(logits, dim=-1)
            if return_all:
                all_logits.append(logits)
...
print(f"\nEXPERIMENT: 2 transformers (2 layers each), 8 iters each")
print(f"Training on {device}...")

log_file = open("exp_middle1.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
