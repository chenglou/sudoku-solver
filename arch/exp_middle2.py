# Experiment: 4 transformers (1 layer each), each handles 4 iterations
# Tests: finer-grained iteration specialization
# Same ~800k params as sudoku.py, but 16 layer passes instead of 64
import torch
...
n_heads = 4
d_ff = 512
n_layers_per_transformer = 1  # 1 layer each (vs 4 in original)
n_transformers = 4
iters_per_transformer = 4     # 4 iterations each, total 16
lr = 1e-3
steps = 100000
n_train = 100000
n_test = 1000
...
class SudokuTransformer(nn.Module):
    def __init__(self):
        ...
        # Four separate transformers with 1 layer each
        ...
        for _ in range(n_transformers):
            ...
            self.transformers.append(
                nn.TransformerEncoder(encoder_layer, num_layers=n_layers_per_transformer)
            )
    ...
    def forward(self, x, return_all=False):
        ...
        # Each transformer handles 4 iterations
        for t_idx, transformer in enumerate(self.transformers):
...
print(f"\nEXPERIMENT: 4 transformers (1 layer each), 4 iters each")
print(f"Training on {device}...")

log_file = open("exp_middle2.log", "w")
def log(msg):
    print(msg)
    log_file.write(msg + "\n")
    log_file.flush()
