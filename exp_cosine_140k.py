# Experiment: Cosine LR decay - 140K steps
# Based on exp_cosine.py (84.0% at 70K)
# Changes:
#   - Double training steps: 70K -> 140K
#   - Slower cosine decay over longer horizon
# Hypothesis: More training with slower LR decay may push accuracy higher
...
CHECKPOINT_PREFIX = "cosine_140k_checkpoint_step"

# Config for checkpoint verification
CONFIG = {
    'experiment': 'exp_cosine_140k',
    'd_model': 128,
    'n_layers': 4,
    'batch_size': 4096,
    'lr': 1.5e-3,
...
warmup_steps = 2000
lr_min_ratio = 0.01
total_steps = 140000  # 2x the original
batch_size = 4096
sam_rho = 0.05
train_size = 2700000

# Reverse curriculum phases - scaled 2x
PHASES = [
    (0, 28000, 21, "Phase 1: Hard only (rating 21+)"),
    (28000, 56000, 6, "Phase 2: Medium+ (rating 6+)"),
    (56000, 84000, 1, "Phase 3: Easy+ (rating 1+)"),
    (84000, 140000, 0, "Phase 4: All (rating 0+)"),
]
...
def train(output_dir="."):
    ...
    print(f"\nExperiment: Cosine LR decay 140K steps (min_ratio={lr_min_ratio})")
    ...
    log_path = os.path.join(output_dir, "exp_cosine_140k.log")
    ...
    log("\n" + "="*60)
    log("FINAL RESULTS - Cosine LR decay 140K")
    log("="*60)
    results = evaluate_all()
    total_r = results.pop('_total')
    ...
    log(f"Cosine 70K baseline: 84.0%")
    log(f"nano-trm: 87.4%")

    final_path = os.path.join(output_dir, "model_cosine_140k.pt")
